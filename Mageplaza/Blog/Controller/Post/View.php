            ...
            if (isset($params['cmtId'])) {
                ...
                if($params['typeButton'] == 'like'){
                    ...
                    $dislikeData = [
                        ...
                        $likeModel = $this->likeFactory->create();
                        $result    = $this->commentActions(self::DISLIKE, $customerData, $dislikeData, $likeModel, $cmtId);
                    }
                }
    ...
    public function commentActions($action, $user, $data, $model, $cmtId = null)
    {
        try {
            switch ($action) {
                ...
                /** DisLike action */
                case self::DISLIKE:
                    $checkDislike = $this->isLikedComment($cmtId, $user->getId(), $model);
                    if (!$checkDislike) {
                        $model->addData($data)->save();
                    }
                    $likes      = $model->getCollection()->addFieldToFilter('comment_id', $cmtId);
                    $countDislikes = $likes->getSize() ?: '';
                    $isDisLiked    = $checkDislike ? 'yes' : 'no';
                    $result     = [
                        'disliked'      => $isDisLiked,
                        'comment_id' => $cmtId,
                        'count_like' => $countDislikes,
                        'status'     => 'ok'
                    ];
                    break;
                default:
