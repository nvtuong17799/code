...
require([
    ...
    var cmtBox = $('.default-cmt__content__cmt-block__cmt-box__cmt-input'),
        ...
        dislikeBtn = defaultCmt.find('.btn-dislike'),
        replyBtn = defaultCmt.find('.btn-reply');

    submitComment();
    likeComment(likeBtn);
    dislikeComment(dislikeBtn);
    showReply(replyBtn);
                ...
                if (isLogged === 'Yes') {
                    ...
                    if ($(this).attr('click') === '1') {
                        ...
                        $.ajax({
                            ...
                            data: {cmtId: cmtId, typeButton: 'like'},
        ...
        btn.each(function () {
            ...
            likeEl.click(function () {
                ...
                if (isLogged === 'Yes') {
                    var likeCount = $(this).find('span').text();
                    if ($(this).attr('click') === '1') {
                        if ($(this).hasClass('mpblog-disliked')) {
                            $(this).css('color', '#333333');
                            likeCount--;
                            $(this).find('span').text((likeCount === 0) ? "" : likeCount);
                            $(this).removeClass('mpblog-disliked')

                        } else {
                            likeCount++;
                            $(this).find('span').text(likeCount);
                            $(this).css('color', likedColor);
                            $(this).addClass('mpblog-disliked')
                        }
                        $.ajax({
                            type: "POST",
                            url: window.location.href,
                            data: {cmtId: cmtId, typeButton: 'dislike'},
                            success: function (response) {
                                if (response.status === 'ok') {
                                    $(likeEl).attr('click', '1');
                                } else if (response.status === 'error' && response.hasOwnProperty(error)) {
    ...
    function displayComment(cmt, isReply) {
        ...
        var cmtRow = '<li style="width: 100%" id="cmt-id-' + cmt.cmt_id + '" class="default-cmt__content__cmt-content__cmt-row cmt-row-' + cmt.cmt_id + ' cmt-row col-m-12 '
            ...
            + cmt.cmt_id + '" click="1"><i class="fa fa-thumbs-up" aria-hidden="true" style="margin-right: 3px"></i><span class="count-like__like-text"></span></a> <a class="interactions__btn-actions action btn-dislike mpblog-dislike" data-cmt-id="'
            + cmt.cmt_id + '" click="1"><i class="fa fa-thumbs-down" aria-hidden="true" style="margin-right: 3px"></i><span class="count-dislike__dislike-text"></span></a> <a class="interactions__btn-actions action btn-reply" data-cmt-id="'
            ...
            replyCmt.each(function () {
                ...
                if (cmtEl.attr('data-cmt-id') === replyCmtId) {
                    ...
                    if (!replyList.length) {
                        ...
                        dislikeComment(cmtRow.find('.btn-dislike'));
