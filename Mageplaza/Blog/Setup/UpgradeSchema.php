    ...
    public function upgrade(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        if (!$this->helperData->versionCompare('2.3.0')) :
            ...
            if (version_compare($context->getVersion(), '2.5.3', '<')) {
                if ($installer->tableExists('mageplaza_blog_category')) {
                    if (!$connection->tableColumnExists($installer->getTable('mageplaza_blog_category'), 'image')) {
                        $connection->addColumn($installer->getTable('mageplaza_blog_category'), 'image', [
                            'type'    => Table::TYPE_TEXT,
                            255,
                            [],
            ...
            if (version_compare($context->getVersion(), '2.5.4', '<')) {
                if ($installer->tableExists('mageplaza_blog_comment_like')) {
                    $installer->getConnection()->addColumn(
                        $installer->getTable('mageplaza_blog_comment_like'),
                        'action',
                        [
                            'type' => Table::TYPE_INTEGER,
                            'size' => null,
