...
class View extends \Mageplaza\Blog\Block\Listpost
{
     ...
     * @return int|string
     */
    public function getCommentLikes($cmtId, $action)
    {
     ...
     * @return bool
     */
    public function isLiked($cmtId, $action)
    {
        if ($this->helperData->isLogin()) {
            ...
            foreach ($likes as $like) {
                if ($like->getEntityId() == $customerId && $like->getCommentId() == $cmtId && $like->getAction() == $action) {
                    return true;
                }
            }
        }
    ...
    public function getCommentsTree($comments, $cmtId)
    {
        ...
        foreach ($comments as $comment) {
            if ($comment['reply_id'] == $cmtId && $comment['status'] == 1) {
                ...
                $countLikes        = $this->getCommentLikes($comment['comment_id'], '1');
                $countDislikes        = $this->getCommentLikes($comment['comment_id'], '0');
                $isLiked           = ($this->isLiked($comment['comment_id'], '1')) ? "mpblog-liked" : "mpblog-like";
                $isDisliked           = ($this->isLiked($comment['comment_id'],'0')) ? "mpblog-disliked" : "mpblog-dislike";
                $this->commentTree .= '<li id="cmt-id-' . $comment['comment_id']
                    ...
                    . $countLikes . '</span></a>
                                        <a class="interactions__btn-actions action btn-dislike '
                    . $isDisliked . '" data-cmt-id="'
                    . $comment['comment_id'] . '" click="1">
                                        <i class="fa fa-thumbs-down" aria-hidden="true"></i>
                                        <span class="count-dislike__dislike-text">'
                    .$countDislikes . '</span></a>
