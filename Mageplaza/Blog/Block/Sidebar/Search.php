...
class Search extends Frontend
{
        ...
        if (!empty($collection)) {
            foreach ($collection as $item) {
                ...
                $result[] = [
                    'value' => $item->getName(),
                    'url'   => $item->getUrl(),
                    'image' => $this->resizeImage($item->getImage(), '100x', $type),
    ...
    public function getSearchAllData(){
        ...
        $post = $this->getSearchObjectData(Data::TYPE_POST);
        $category = $this->getSearchObjectData(Data::TYPE_CATEGORY);
        $topic = $this->getSearchObjectData(Data::TYPE_TOPIC);
        $tag = $this->getSearchObjectData(Data::TYPE_TAG);
